...
def plan_path(start, goal, obstacles, width, height, margin=3):
    ...
    print([-int(width / 2), int(width / 2), -int(height / 2), int(height / 2)])
...
# Kick 1 pos:
# position=(-186.67307, -466.31915), orientation=-0.6791174
dest = np.array([-186.67307, -466.31915])
target_orientation = -0.6791174
...
if __name__ == "__main__":
    ...
    heading_pid = PID(dim=1, Kp=2, Ki=0.002, Kd=0.0, diff_func=angle_diff)
    heading_pid.set_target(target_orientation)

    pos_pid = PID(dim=2, Kp=0.7, Ki=0.0, Kd=0.0)
    traj_pid = PID(dim=2, Kp=1.2, Ki=0.0, Kd=0.0)
    ...
    to_save = []
    last_time = time.time()
    bridge.send(PlayerCmd(player_id, 0, 0, 0, 0, arm=True))
    try:
        while True:
            msg = bridge.recv()
            if not msg:
            ...
            if not has_ball and not waiting:
                target = path[path_idx]
                dist = np.linalg.norm(target - pos)
                if path_idx == len(path) - 1:
                    tresh = 80
                    pos_pid.Ki = 0.0002
                    print("Last dist: ", dist)
                else:
                    tresh = 300
                if dist < tresh:
                    ...
                    if path_idx >= len(path):
                        ...
                        waiting = True
                        print("Waiting")
                        bridge.send(PlayerCmd(player_id, 0, 0, 0, arm=True))
                        continue
                    target = path[path_idx]
            ...
            elif waiting and not has_ball:
                if msg.ball is not None:
                    ball_pos = np.array([ball.position[0], ball.position[1]])
                    ball_dist = np.linalg.norm(ball_pos - pos)
                ...
                if abs(phi_err) <= 0.05:
                    print("Kicking!")
                    bridge.send(
                        PlayerCmd(player_id, 0, 0, dribble_speed=dribble, disarm=True)
                    )
                    time.sleep(0.05)
                    bridge.send(
                        PlayerCmd(player_id, 0, 0, dribble_speed=dribble, arm=True)
                    )
                    time.sleep(0.4)
                    bridge.send(
                        PlayerCmd(player_id, 0, 0, dribble_speed=dribble, kick=True)
                    )
                    time.sleep(0.1)
                    bridge.send(
                        PlayerCmd(player_id, 0, 0, dribble_speed=dribble, kick=True)
            ...
            u /= 1000.0
            vx, vy = global_to_local_vel(float(u[0]), float(u[1]), phi)
            ...
            if not has_ball and not waiting:
                angle_thresh = 0.4 if path_idx == len(path) - 1 else 0.05
                if abs(phi_err) > 0.2:
                    heading_pid.Ki = 0.02
