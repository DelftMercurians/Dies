...
pub async fn start_server() -> Result<(), Box<dyn std::error::Error>> {
    ...
    async fn handle_conn(conn: Stream) -> io::Result<()> {
        ...
        // Serialize StrategyInput to JSON
        let serialized_input =
            serde_json::to_vec(&strategy_input).expect("Failed to serialize input");
        ...
        // Deserialize the received data into StrategyOutput
        let strategy_output: StrategyOutput =
            serde_json::from_slice(&buffer).expect("Failed to deserialize output");

        // Produce our output!
        println!("Client answered: {}", strategy_output.message);
        Ok(())
    }
