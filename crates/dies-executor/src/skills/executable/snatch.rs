...
const DRIBBLER_SPEED: f64 = 0.8;
/// Max distance (mm) from an opponent's centre at which the ball still counts as
/// "held" by that opponent (centre-to-ball; the held ball sits ~150mm in front).
const HOLD_RANGE: f64 = 260.0;
/// Half-angle (rad) of the cone in front of the opponent within which the ball
/// counts as held. Generous — we only need to identify the holder.
const HOLD_CONE: f64 = 0.9;
...
const CONTACT_DIST: f64 = 190.0;
/// How close to the contact pose (mm) before we commit to pressing + rotating.
const ENGAGE_TOL: f64 = 80.0;
/// Heading tolerance (rad) to be considered aligned for engagement.
const ENGAGE_ANGLE: f64 = 0.5;
/// Slow in-place rotation rate (rad/s) once engaged — the peel itself.
const ROT_RATE: f64 = 1.3;
/// Speed cap (mm/s) while approaching, so contact stays gentle (no crash foul).
const APPROACH_SPEED_LIMIT: f64 = 700.0;
