    ...
    fn update(&mut self, ctx: RoleCtx<'_>) -> PlayerControlInput {
        if let (Some(ball), Some(geom)) = (ctx.world.ball.as_ref(), ctx.world.field_geom.as_ref()) {
            ...
            if self.distance(ctx.player.position, ball.position.xy()) < 500.0 {
                ...
                println!(
                    "Distance to starting pos: {}",
                    self.distance(self.starting_pos, ctx.player.position)
                );
            ...
            if ctx.player.id == self.shooter_id.unwrap() {
                ...
                if self.distance(shooter_pos, self.position) < 100.0
                    && self.distance(shooter_pos, ball.position.xy()) < 280.0
                {
