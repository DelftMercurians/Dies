...
use super::bootstrap::{ensure_self_venv, fetch, get_pip_module};
use super::consts::VENV_BIN;
use super::lock::{
    make_project_root_fragment, update_single_project_lockfile, update_workspace_lockfile,
    LockMode, LockOptions,
};
use super::piptools::get_pip_sync;
use super::platform::get_toolchain_python_bin;
use super::pyproject::{get_current_venv_python_version, ExpandedSources, PyProject};
use super::sources::PythonVersion;
use super::utils::{get_venv_python_bin, set_proxy_variables, symlink_dir, CommandOutput};
...
pub fn sync(cmd: SyncOptions) -> Result<(), Error> {
    ...
    if venv.is_dir() {
        if let Some(marker_python) = get_current_venv_python_version(&venv) {
            ...
            if cmd.output != CommandOutput::Quiet {
                info!("Forcing re-creation of non rye managed virtualenv");
            }
            recreate = true;
    ...
    if venv.is_dir() {
        ...
        {
            info!("Reusing already existing virtualenv");
        }
    } else {
        if output != CommandOutput::Quiet {
            ...
            info!("Python version: {}", style(&py_ver).cyan());
        }
