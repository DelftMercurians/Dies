...
use super::sources::{get_download_url, PythonVersion, PythonVersionRequest};
use super::utils::{
...
pub fn ensure_self_venv(output: CommandOutput) -> Result<PathBuf, Error> {
    ...
    if venv_dir.is_dir() {
        if is_up_to_date() {
            ...
            if output != CommandOutput::Quiet {
                info!("detected outdated rye internals. Refreshing");
            }
    ...
    if output != CommandOutput::Quiet {
        info!("Bootstrapping rye internals");
    }
...
fn do_update(output: CommandOutput, venv_dir: &Path, app_dir: &Path) -> Result<(), Error> {
    ...
    if output != CommandOutput::Quiet {
        info!("Installing internal dependencies");
    }
    if output == CommandOutput::Verbose {
        pip_install_cmd.arg("--verbose");
    } else {
...
pub fn fetch(
    ...
    if let Ok(version) = PythonVersion::try_from(version.clone()) {
        ...
        if py_bin.is_file() {
            if output == CommandOutput::Verbose {
                info!("Python version already downloaded. Skipping.");
            }
            return Ok(version);
        }
    }
    ...
    if output != CommandOutput::Quiet {
        info!("{} Downloaded {}", style("success:").green(), version);
    }
