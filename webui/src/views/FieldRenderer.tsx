...
export class FieldRenderer {
  ...
  private WorldFrame: WorldFrame | null = null;
  ...
  setWorldFrame(WorldFrame: WorldFrame | null) {
  ...
  render(selectedPlayerId: number | null, manualControl: number[]) {
    ...
    const { own_players, opp_players, ball } = this.WorldFrame;
  ...
  private drawFieldLines() {
    if (!this.WorldFrame?.field_geom?.line_segments) return;

    this.ctx.lineWidth = 1;
    this.WorldFrame.field_geom.line_segments.forEach(({ p1, p2 }) => {
  ...
  getPlayerAt(x: number, y: number): number | null {
    if (!this.WorldFrame) return null;

    const { own_players, ball } = this.WorldFrame;
