    ...
    let new_state = match state {
        Some(existing_state) => {
            ...
            if should_change_zone {
                ...
                let current_target = get_zone_target_position(situation, &existing_state.current_zone);
    ...
    select_node()
        .add(
            ...
            guard_node()
                ...
                .condition(|s| {
                    s.game_state_is_one_of(&[dies_core::GameState::PrepareKickoff, dies_core::GameState::Kickoff])
                })
                .then(
...
fn score_zone_for_player(situation: &RobotSituation, zone: &str) -> f64 {
    ...
    // Congestion penalty
    let congestion = situation.get_opp_players_within_radius(zone_center, 1500.0).len() as f64;
...
fn fetch_and_shoot() -> BehaviorNode {
    ...
    go_to_position(|s| s.world.ball.as_ref().map(|b| b.position.xy()).unwrap_or_default())
        .build()
        .into()
}
